...
This feature is available to all Microsoft 365 customers. The following admin roles can setup, view monitored conversations and act on them:  
...
You can use regular expressions for pattern matching. The following table lists examples:

| Purpose | Pattern or phrase | Matching values example |
| ------- | ------------------| ----------------------- |
...
| Monitor group creation | has created | Matthew has created the Easter Region Sales group. |

### Define email notifications for keyword alerts

When a post in Viva Engage matches a monitored keyword, an email alert is sent to the specified address. The notifications help admins and moderators stay informed and take timely action on potentially sensitive or relevant content. Alert content includes the following fields:
...
Public Community or Storyline Posts:

the email notification includes the full conversation and all associated replies, and lists the name of the community or storyline for context.

Private Community Posts:

The email includes a direct link to the conversation and the private community where it was detected. Only members of the community can view the conversation by selecting the link.

To enhance security and compliance, Viva Engage includes the following safeguards:

Tenant Email Restrictions include the following:

- Only Entra ID–backed email addresses from within your Microsoft 365 tenant are supported as keyword alert recipients.
- Consumer email addresses (Gmail, Yahoo, Outlook.com) are not supported.
- If an unsupported email is entered, the user will be prompted to provide a valid tenant-based address.

Exclusion of Private Messages:

- Keyword detection does not apply to private messages between individuals.

- These messages are confidential and must be accessed through approved Microsoft 365 compliance tools such as eDiscovery when needed.
